...
The framework adapts the Tiebout model to the platform context. The full formal specification is in Appendix~\ref{app:model}; this section conveys the essential logic in plain terms and introduces the four core equations that drive the results.
...
All communities derive base utility from the match between their preferences and the governance bundle their current platform offers:
...
\noindent where $\delta$ is the Kronecker delta, $b_{pa}$ is the platform's bundle on policy dimension $a$, and $b_{ca}$ is the community's preference on that dimension. The remaining three equations modify this base to capture social exposure.

Mainstream communities want platforms whose governance bundles align with their preferences. When no extremist communities are present, their utility is simply $u_{c_m} = u_{\text{base}}(c_m, p)$. When extremist communities arrive, mainstream communities suffer a utility penalty proportional to their exposure. The mainstream utility function is:
...
Governance type determines sorting efficiency, but the ranking inverts between uniform and mixed systems: algorithmic platforms dominate in homogeneous configurations, yet direct-voting platforms deliver the highest per-capita utility once communities can sort freely across governance types.
...
Taken together, the five result subsections establish a coherent account of how decentralized sorting produces the concentration--raid--retreat cycle. Platform diversification protects mainstream communities and does so more strongly as parasitism intensifies, shifting the mechanism from preference matching to target dilution. Coalition governance provides the most durable structural firewall through endogenous enclave formation, while direct-voting platforms shift from high-performing boutique spaces to exploitation targets as parasitism rises. The concentration of extremists on direct platforms (50\%, stable across all factor levels), the bursty character of their movement (94--100\% of outflow in burst events, median burst size 31), and the sharp welfare divergence between governance types ($-3.60$ vs.\ $-0.22$ utility decline) are all emergent properties of decentralized utility maximization, not the product of strategic coordination. These patterns hold under mixed ideologue--griefer populations (§\ref{sec:results_mixed}), confirming that the $\alpha$ parameterization captures the relevant behavioral variation without qualitative distortion.
...
The agent-based model code, experiment harness, analysis scripts, and figure-generation scripts required to reproduce the simulations reported in this manuscript will be made available in a public repository: \url{[REPOSITORY URL TO BE INSERTED BY AUTHOR]}. The core ABM implementation is in \texttt{platform\_abm/}, including the community and platform agents, utility functions, search and relocation logic, governance institutions, tracking, and post-hoc analyzers. The experiment specifications and execution harness are in \texttt{experiments/configs/}, \texttt{experiments/runner.py}, and the experiment entry points \texttt{experiments/run\_exp1.py}, \texttt{experiments/run\_exp2.py}, \texttt{experiments/run\_exp2b.py}, \texttt{experiments/run\_sensitivity.py}, and \texttt{experiments/run\_biography\_configs.py}. Analysis modules used to summarize relocation, burst, enclave, convergence, displacement, and effect-size outputs are in \texttt{platform\_abm/analyzer.py}, \texttt{platform\_abm/burst\_analysis.py}, \texttt{platform\_abm/tracker.py}, \texttt{results/analyze\_stepwise.py}, \texttt{results/burst\_analysis.py}, \texttt{results/displacement\_diagnostic.py}, \texttt{results/effect\_sizes.py}, and \texttt{results/extract\_distributions.py}. Figure-generation and figure-preparation scripts are in \texttt{results/visualize\_exp2b.py}, \texttt{results/visualize\_exp2b.R}, \texttt{results/visualize\_biographies.R}, \texttt{results/visualize\_distributions.R}, and \texttt{results/viz/}. Generated experiment outputs and manuscript figures are organized under \texttt{results/}. Reproducibility controls include deterministic per-iteration seeding from \texttt{seed\_base}, single-thread BLAS settings in worker processes, skip-if-done provenance tracking in \texttt{index.json}, and crash-recovery append behavior for partially completed configuration runs.
\newpage
