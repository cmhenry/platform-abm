...
\author[1]{Colin Henry}
% \author[2,3]{Second Author}
% \author[2]{Third Author}
\affil[1]{Department of Political Science, University of Zurich, Switzerland}
...
\begin{abstract}
How do political communities move between social media platforms? This paper develops and tests a modified Tiebout model of community sorting across online platforms, conceptualizing platforms as quasi-governments offering public goods in the form of attention distribution to user-generated content. Using agent-based computational simulations, I examine how different platform governance structures---direct voting, coalition formation, and algorithmic recommendation---affect the distribution of utility among communities. The results demonstrate that platform consolidation, extremist presence, and governance mechanisms significantly shape community outcomes. Extremist communities act as ``utility vampires,'' extracting utility from mainstream communities through hostile action. The simulations reveal that an online informational environment with few platform options, vague content moderation policies, and limited community autonomy drives political communities toward radicalization, while platform diversity and coalition-based governance can mitigate these effects. \\
...
Within this framework, platform-system structure shapes extremist strategy and, through it, the distribution of attention and safety for mainstream communities. Unlike classic citizen-sorting models, extremists can generate utility by imposing costs on other groups. In the simulations, extremist communities act as ``utility vampires,'' extracting utility from mainstream communities through hostile action. As a result, some platform ecosystems generate a stable cycle of extremist concentration, raiding, and re-entry, while others dampen it.
...
% ============================================================
% STRUCTURE NOTES (remove before submission)
% 
% Narrative arc: Tiebout baseline works → extremists break it in 
% specific ways → the breaks reveal something about platform system design.
%
% Experiment 1 establishes two baselines that Experiment 2 complicates.
% Experiment 2 is the main event: four cross-sectional findings plus
% a dynamics subsection on raiding.
% ============================================================

\subsection{Experiment 1: Sorting Without Extremists}
\label{sec:results_exp1}

% This section should be relatively compact — the machinery section,
% not the payoff. Two results that set up the question Experiment 2 answers.

\subsubsection{Result 1: Governance type shapes sorting efficiency, but the ranking depends on system composition}

% In homogeneous systems (single governance type across all platforms):
%   - Algorithmic: 6.49 avg utility (highest)
%   - Coalition: 6.06
%   - Direct: 5.40 (lowest)
% Gap between algorithmic and direct exceeds 1.0 (>10% of max).
% Mechanism: algorithmic platforms serve heterogeneous preferences 
% simultaneously via per-group bundles. Direct forces single majority-rule 
% bundle — losers eat full cost.
%
% In mixed systems, the ranking PARTIALLY INVERTS:
%   - Direct platforms deliver highest per-capita utility 
%     (6.31 at N_p=3, 7.05 at N_p=9, 7.61 at N_p=27)
%   - Mechanism: most communities (59–76%) evacuate to algorithmic platforms.
%     Residual population on direct platforms is self-selected for preference
%     alignment. Direct voting works beautifully when electorate agrees.
%   - Selection effect strengthens with system size: at N_p=27, direct platforms 
%     hold 4.8% of communities but deliver utility 1.0+ above system average.
%
% Coalition platforms stable at 19–20% across all N_p, utility 5.84–6.54.
% Serve niche: too heterogeneous for direct, prefer explicit coalition 
% structure over opaque algorithms.
%
% Key tables: homogeneous comparison (3 rows), mixed comparison by N_p (3×3).
% Convergence diagnostics in footnote: utility converges within 10–15 steps; 
% behavioral convergence slower (lateral shuffling).

\subsubsection{Result 2: Platform diversification improves welfare}

% System-wide utility increases monotonically: 
%   6.20 (N_p=3) → 6.42 (N_p=9) → 6.63 (N_p=27)
% Every governance type benefits. Variance decreases (SD 0.144 → 0.116).
% More platforms produce higher AND more equal outcomes.
% Gain diminishing (~0.2 per tripling) but consistent.
%
% This is the core Tiebout prediction confirmed in the platform context.
% Sets up the Experiment 2 question: does this hold when some communities 
% are parasitic?

\subsection{Experiment 2: Extremist Dynamics in Mixed Platform Systems}
\label{sec:results_exp2}

% Main event. 3×3×3 factorial: N_p ∈ {3,9,27}, ρ_e ∈ {0.05,0.10,0.15}, 
% α ∈ {2,5,10}. N_c=900, N_a=10, t_max=100, μ=0.05, mixed governance 
% (equal thirds). 200 iterations per config.

\subsubsection{Result 3: Diversification protects mainstream communities, and the protection premium grows with extremist threat}

% Central quantitative finding. Frame around N_p × α interaction at ρ_e=0.15:
%
% Normalized mainstream utility:
%                   N_p=3    N_p=9    N_p=27   Δ(27 vs 3)
%   α=2             0.593    0.619    0.642    +0.049
%   α=5             0.562    0.600    0.630    +0.068
%   α=10            0.511    0.567    0.609    +0.098
%
% Diversification premium nearly doubles from low to high parasitism.
% Difference-in-differences: (0.098 - 0.049) = 0.049 on normalized scale.
%
% Mechanism change from Experiment 1: no longer just preference matching 
% but also target dilution. More platforms spread extremists thinner, 
% reducing parasitism density on any given platform.
%
% CENTRAL CLAIM: Platform system structure and extremist behavior are not 
% independent. The welfare cost of consolidation depends on what kind of 
% extremists inhabit the system, and the benefit of diversification is 
% greatest precisely when the extremist threat is most severe.

\subsubsection{Result 4: Parasitism intensity produces qualitatively distinct extremist regimes}

% The α parameter changes WHAT extremists DO, not just how much they earn.
%
% Ideologue regime (α=2): 
%   - Extremists on direct platforms earn 10.4–11.3 utility
%   - Most payoff from policy alignment (left side of utility function)
%   - Mainstream on same direct platforms still earn 5.6–7.2 — functional
%
% Griefer regime (α=10):
%   - Extremists on direct platforms earn 12.7–17.8
%   - Parasitism bonus dominates
%   - Mainstream on direct collapses to 3.41 (worst config) — BELOW random baseline of 5.0
%
% The regime boundary falls between α=5 and α=10 in most configurations.
% Evidence: direct/algorithmic utility ratio for extremists grows with α.
%
%                              Ext.Direct  Ext.Algo  Ratio
%   α=2,  N_p=9, ρ=0.10       10.90       6.91      1.58
%   α=5,  N_p=9, ρ=0.10       12.77       7.74      1.65
%   α=10, N_p=9, ρ=0.10       15.69       9.42      1.67
%
% Connection to extremism literature: ideologue/griefer distinction is not 
% a binary type assignment but an emergent behavioral regime driven by the 
% payoff structure.

\subsubsection{Result 5: Coalition governance creates protective enclaves}

% Coalition platforms deliver most stable mainstream utility across α range:
%   6.35 → 6.13 (decline of 0.22 as α: 2→10)
%   vs. direct: 7.01 → 3.41 (decline of 3.60)
%
% Mechanism: enclave formation.
%   - Mean coalition homogeneity: 0.92–0.98
%   - 74–92% of simulation steps above 0.9 enclave threshold
%   - Segregation completes within 25–30 steps
%   - Once extremists and mainstream occupy separate coalitions, parasitism 
%     channel is severed
%
% Reframes Experiment 1 finding: coalition platforms looked mediocre 
% without extremists. Their movement formation mechanism turns out to 
% be a natural firewall against parasitic neighbors.
%
% Temporal pattern from enclave data: early turbulence (steps 1–25), 
% then sustained near-perfect homogeneity. Rare disruptions coincide 
% with incoming raids from direct platforms; coalition mechanism re-sorts 
% within a few steps.

\subsubsection{Result 6: Extremists concentrate on direct platforms and exploit the direct–algorithmic interface}

% ~50% of extremists end up on direct platforms regardless of system parameters.
% Direct platforms hold only 5–23% of all communities.
% Concentration stable across all factor levels:
%   ρ_e=0.05: 22.1/45 on direct (49%)
%   ρ_e=0.10: 44.9/90 on direct (50%)
%   ρ_e=0.15: 67.4/135 on direct (50%)
%
% Mechanism: combination of capturable majority-rule governance and 
% unprotected mainstream neighbors. Direct platforms offer no structural 
% protection — all communities are neighbors under direct voting.
%
% This concentration is the setup for the raiding dynamics below.

\subsection{Raiding Dynamics}
\label{sec:results_raiding}

% Separate subsection: most novel finding, requires different evidence 
% (time series vs cross-sectional averages). Replaces the qualitative 
% network graph descriptions in the existing draft with proper statistics.

\subsubsection{Result 7: Extremist movement follows a bursty raiding pattern}

% On direct platforms, 94–100% of extremist outflow occurs in burst events 
% (≥10 departures in a single step).
%   - Median burst: 31 extremists
%   - Max burst: 74
%   - Median inter-burst interval: 9 steps
%   - ACF detection: 3/27 platforms; burst detection: 18/27
%
% The mechanism: extremists accumulate on direct platform → shift bundle 
% toward polarized preferences → extract vampire utility from remaining 
% mainstream → mainstream gradually departs → balance tips → large group 
% simultaneously discovers better base utility elsewhere → departs en masse 
% → arrives on algorithmic platform → temporarily disrupts SVD groupings 
% (cold-start problem) → extracts utility from mixed groups → algorithm 
% re-sorts → cycle restarts.
%
% PENDING: Aggregated escalation test from full 200-iteration pipeline.
% If mean slope > 0 for high-α configs: "raids escalate over the simulation"
% If null: "raids are bursty and large but don't systematically grow — 
% steady-state cycle rather than runaway process"
%
% Coalition contrast: near-zero outflow after first 25 steps. Rare 
% disruptions from incoming raids restore to high homogeneity within 
% a few steps. Dynamics-level evidence for the enclave finding.

\subsection{Summary}
\label{sec:results_summary}

% Three-sentence synthesis:
% (1) The Tiebout mechanism works for platform sorting but produces 
%     asymmetric outcomes when communities have parasitic utility functions.
% (2) System structure (N_p) and governance design interact with extremist 
%     behavior (α) to determine mainstream welfare.
% (3) The dynamics of extremist movement reveal emergent raiding patterns 
%     that no individual community plans but that the system structure produces.
%
% Transition to discussion.

% \section{Discussion}

% Results from the modified Tiebout simluation experiments show that platform dynamics are more complicated than previously suggested. Community interactions, especially across platforms with different regime types, have the potential to radicalize or provide substantial utility gains to extremist communities. These results from the simulation experiments should be interpreted as a device for building theory and directing empirical inquiry. We are left with the intuition that communities are competing for public goods on these platforms, different types of platforms deliver goods in more or less efficient methods, and communities obtain utility from competition in different ways. Integrating these intuitions into existing theories to produce testable implications is our next task. 
...
%  Second, it suggests that extremist communities may profitably pursue strategies of frequent movement between platforms. Journalists working on the online extremist beat have long published stories on harassment or hate campaigns organized off-platform---see, for example, the case of Kiwi Farms, a small ``direct"-type platform that became famous for organizing violent harassment campaigns in mainstream spaces. Many possible motivations for this behavior have been suggested: recruitment, ``trolling," persuasion, and so forth. Simulation results suggest that the motivation or strategy for extremist movement between platforms may depend on the social media structure itself.
